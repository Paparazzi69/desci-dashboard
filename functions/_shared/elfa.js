...
//
// All other code MUST go through elfaGet · no direct fetch to api.elfa.ai
// elsewhere in the repo. The cap and the audit trail depend on it.

const ELFA_BASE = 'https://api.elfa.ai';

// Endpoints that cost more than the default 1 credit per call. Source:
// Elfa pricing docs · keep in sync if Elfa changes their pricing.
...
export async function elfaGet(env, path, params = {}) {
  if (!env.ELFA_API_KEY) throw new Error('ELFA_API_KEY env var not set');

  // Pre-flight: refuse if we've already burned through the month's quota.
  ...
  // 429 · honor Retry-After once, then retry.
  if (res.status === 429) {
  ...
  if (!res.ok) {
    ...
    await logCall(env, {
      ...
      endpoint: path,
      params: paramsJson,
      // 4xx/5xx still consumes a credit on most APIs, including Elfa per
      // their docs. Log it so the cap stays honest.
      credits_used: cost,
      http_status: res.status,
      success: false,
      error_msg: `HTTP ${res.status}: ${body}`,
    });
    const err = new Error(`Elfa ${res.status} for ${path}: ${body}`);
